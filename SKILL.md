...
### SOP — generacja `artifacts/sops/{slug}.md`

Wyciągnij z Notion entry: `Process slug`, `Title`, `Trigger`, `Steps`, `Outputs`, `Definition of Done` + observed fields.

Struktura pliku:
```markdown
# {Title bez prefix}

> Status: Draft (z auto-generacji, {Date})
> Owner: {Owner}
> Frequency: {Frequency}
> Executor: {Executor target overall}
> Source: {Source URL}
...
## Steps
{Steps — każdy krok z executorem i outputem}

## Outputs
{Outputs}

## Definition of Done
{Definition of Done — jako checklist}

{Dla każdego observed field obecnego w entry: ## {Field name} → wartość}

---
<!-- Auto-generated by knowledge-base scan {Date}. Template derived from {N} prior SOPs ({field_coverage}% coverage). -->
```

Puste pole z template'u → placeholder `[TBD: opisz {field}]` + komentarz `<!-- wymagane od owner -->`.

### n8n — generacja `artifacts/n8n/{flow_name}.json`

Z Notion entry buduj n8n workflow skeleton — minimalny JSON z węzłami:
...
Maciek importuje JSON do n8n cloud, dopina credentials, testuje wg `meta.test_plan`.

### Skill — generacja `artifacts/skills/{skill_name}/SKILL.md`

Format zgodny z konwencją skill-creator (frontmatter + body). Z Notion entry:
...
## Kontekst
{Description rozwinięty}

## Input format
{Input format z Notion}

## Output format
{Output format z Notion}

## Examples
{Examples z Notion — 2-3 pary input/output}

## Style guide
{Persona/style guide z Notion}

## Edge cases
{Edge cases z Notion}

## Related skills
{Related skills z Notion}

---
<!-- Auto-generated by knowledge-base scan {Date}. Source: {Source URL}. -->
```
