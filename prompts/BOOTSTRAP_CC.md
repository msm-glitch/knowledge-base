...
✅ **Source URL WYMAGANE:**
- Slack: permalink / Gmail: link wątku / Drive: viewUrl
- Claude Code: `~/.claude/projects/[cwd]/[session].jsonl` + timestamp
- Brak → wpisz `—`

✅ **User = autor wzorca, NIE skanujący.** Mapuj email na Notion Person ID z config/notion.yaml → users. ≥3 osoby → "(team-wide)" w Title.

✅ **Date = data ORYGINALNEGO zdarzenia** (timestamp z JSONL / data Slack msg / etc.), nie dziś.
